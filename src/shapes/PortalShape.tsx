...
export class PortalShapeUtil extends BaseBoxShapeUtil<PortalShape> {
  ...
  component(shape: PortalShape) {
    ...
    const { channels: sessionUserChannels, loading: sessionChannelsLoading } = useSessionUserChannels({ autoFetch: false })

    // Connection management hook
    const connectionManager = useConnectionManager({
      source: channelId && channel ? { type: 'channel', id: channelId, slug: channel } : null,
      existingConnections: connections ?? [],
      userChannels: sessionUserChannels,
      isActive: isSelected && !!channel,
    })
      ...
      >
        ...
        {connectionManager.showConnectPopover && (
          <ConnectPopover
            ...
            channelsLoading={sessionChannelsLoading}
            position={{
              x: w + sideGapPx + 16/z, // Right edge of shape + gap
              y: 36 + 8/z // Below plus button + small gap
            }}
            z={z}
          />
        )}
